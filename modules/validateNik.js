/* ============================================================================
   modules/validateNik.js  —  deterministic eKTP NIK structure validator (PARIKSA)
   Ported verbatim (logic unchanged) from reference/validate-nik_reference.js to
   an ES module. Pure, deterministic, no network.

   NIK layout (16 digits): PP RR SS DD MM YY NNNN
     PPRRSS = kecamatan code (first 6 digits) -> wilayah_nik.json
   ...
   Verdict: any FAIL -> Inconsistent; else any WARN -> Consistent with warnings;
   else Consistent.

   IMPORTANT: this is a screening aid for a human, never an automated credit
   decision and never proof a document is genuine.
   ============================================================================ */
...
function regionNameMatch(printed, resolved, level) {
  ...
  const a = level === "provinsi" ? normalize(printed) : stripAdmin(printed);
  const b = level === "provinsi" ? normalize(resolved) : stripAdmin(resolved);
  if (!a || !b) return false;
...
/**
 * @param {string} nikRaw   the 16-digit NIK
 * @param {object} printed  { jenis_kelamin, tanggal_lahir (dd-mm-yyyy), provinsi, kabupaten_kota, kecamatan }
 * @param {object} dataset  { provinsi, kabupaten_kota, kecamatan } from regionData
 * @returns {{nik, verdict, decoded, checks}}
 */
export function validateNik(nikRaw, printed, dataset) {
  printed = printed || {};
  ...
  if (!printed.tanggal_lahir || !/^\d{2}-\d{2}-\d{4}$/.test(printed.tanggal_lahir)) {
    ...
    add(
      ...
      match ? "PASS" : "FAIL",
      match
        ? "NIK date matches the printed birth date."
  ...
  if (!kecName) {
    ...
    const mismatches = pairs.filter(
      ([lvl, p, r]) => p && regionNameMatch(p, r, lvl === "provinsi" ? "provinsi" : "other") === false
    );
    ...
    if (mismatches.length) {
      add(
        ...
        "WARN",
        "Printed name differs from resolved: " +
          mismatches.map(([lvl, p, r]) => `${lvl} "${p}" vs "${r}"`).join("; ") +
          "."
      );
    } else {
      add(
        ...
        "Region name consistency",
        "PASS",
        "Printed names match the resolved region" +
