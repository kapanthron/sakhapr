# PRIVACY.md — Morby + PARIKSA (as built)

**Purpose.** Data-protection control mapping for the Morby prototype **as actually
...
**What changed from the original design (read this first).** The earlier PARIKSA/Morby
...
| **Keamanan** (security) | HTTPS everywhere; browser CSP `connect-src 'self'`; admin auth via HMAC-signed HttpOnly cookie; R2 encrypted at rest by Cloudflare. | Admin credentials are **POC values in config** (`pocadmin`/`poc2026#`); no rate limiting; no per-record encryption. |
...
Morby (as built) processes regulated personal data partly on-device (OCR, NIK check) and partly
